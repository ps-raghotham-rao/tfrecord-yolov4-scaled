...
def parse_args(args):
    ...
    parser.add_argument('--class-names', default='chess.names', help="voc.names,coco.names")
...
def generator_to_tfrecord_creator(train_generator):
    ...
    with tf.io.TFRecordWriter("dataset.tfrecords") as writer:
        ...
        for batch_index, (batch_imgs, batch_labels) in train_generator_tqdm:
            width,height,channel=batch_imgs[0].shape
            # batch_img_tensor=tf.convert_to_tensor(batch_imgs[0])
            # batch_label0_tensor=tf.convert_to_tensor(batch_labels[0])
            # batch_label1_tensor=tf.convert_to_tensor(batch_labels[1])
            # batch_img_feature = tf.train.Feature(float_list=tf.train.FloatList(batch_img_tensor))
            # batch_label0_tensor=tf.train.Feature(float_list=tf.train.FloatList(batch_label0_tensor))
            # batch_label1_tensor=tf.train.Feature(float_list=tf.train.FloatList(batch_label1_tensor))
            bytes_buffer = io.BytesIO()

            img = (batch_imgs[0] * 255 / np.max(batch_imgs[0])).astype('uint8')
            ...
            features=tf.train.Features(feature={
                "image": image_feature,
                "label0": _bytes_feature(serialized_label0),
                "label1": _bytes_feature(serialized_label1)
            })
            )
    ...
    "label0": tf.io.FixedLenFeature([], tf.string),
    ...
    def _bytestring_to_pixels(parsed_example):
        ...
        #TODO doubt ide next two lines
        image = tf.cast(image, tf.float32)
        image=image/255.0
        
        parsed_label0=tf.io.parse_tensor(parsed_example['label0'],out_type=tf.float32)
        parsed_label1=tf.io.parse_tensor(parsed_example['label1'],out_type=tf.float32)
    ...
    dataset = load_and_extract_images("dataset.tfrecords")
    return dataset
...
def main(args):
    ...
    if args.train_mode == 'fit':
        ...
        # train_generator_tqdm = tqdm(enumerate(train_generator), total=len(train_generator))
        # for batch_index, (batch_imgs, batch_labels) in train_generator_tqdm:
        train_generator_tqdm=generator_to_tfrecord_creator(train_generator)
        #training
